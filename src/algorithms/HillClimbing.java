...
public class HillClimbing {
    ...
    private void simple_HC(Node current) {
        ...
        double current_Eval = current.getEval();
        double EVal = Double.MAX_VALUE;
        Node Current_node = null;
        step++;
        seenNode += current.getChildren().size();

        for (int i = 0; i < current.getChildren().size(); i++) {

            if (current.getChildren().get(i).getEval() < EVal) {
                EVal = current.getChildren().get(i).getEval();
                Current_node = current.getChildren().get(i);
            }
        }
    ...
    private void stochastic_HC(Node current) {
        ...
        for (int i = 0; i < current.getChildren().size(); i++) {
            ...
            if (current.getChildren().get(i).getEval() < current_Eval) {
                nodes.add(current.getChildren().get(i));
            }
        }
