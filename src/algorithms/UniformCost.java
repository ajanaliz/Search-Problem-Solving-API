...
public class UniformCost {
		...
		while (!open_list.isEmpty()) {
			...
			for (int i = 0; i < last.getChildren().size(); i++) {
				...
				for (int j = 0; j < close_list.size(); j++) {
					if (close_list.elementAt(j).isSame(last.getChildren().elementAt(i)))
						what = false;
				}
				...
				if (what) {
					...
					for (int j = 0; j < open_list.size(); j++) {
						if (open_list.elementAt(j).isSameSetGcost(last.getChildren().elementAt(i)))
							why = false;
					}
					if(why){
	...
	private void showResult(Node node) {
		...
		System.out.println("this is count of observed nodes : " + seenNodes);
