...
public class EightPuzzleNode extends Node {
	...
	public EightPuzzleNode(int row , int col  , int Gcost , Node parent , Problem my_parent, Vector <Vector<Integer>> board, int depth){
...
//		this.print();
	...
	public Vector<Node> getChildren() {
		...
		if(getCol()>0){
			...
			EightPuzzleNode n = new EightPuzzleNode(row, col-1, this.Gcost+1, this, my_parent, board2, this.depth+1);
			child.add(n);
		}
		...
		if(getRow()>0){
			...
			board2.elementAt(row).set(col, x);
			EightPuzzleNode n = new EightPuzzleNode(row-1, col, this.Gcost+1, this, my_parent, board2, this.depth+1);
			child.add(n);
	...
	public boolean isSame(Node e) {
		EightPuzzleNode node = (EightPuzzleNode) e;
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if(board.elementAt(i).elementAt(j)!=node.getBoard2().elementAt(i).elementAt(j))
					return false;
			}
		}
			return true;
